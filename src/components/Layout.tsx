...
export default function Layout({ children, currentPage, setPage }: LayoutProps) {
  ...
  const handleSetPage = (page: string) => {
    setPage(page);
    setIsMobileMenuOpen(false); // Close menu on page change for mobile
  };
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        <div className="lg:hidden bg-white/80 backdrop-blur-md border-b border-[#E5E5E5] p-4 flex items-center justify-between sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-serif font-bold text-lg leading-none">Nihongo</h1>
              <p className="text-[10px] text-[var(--color-ink-light)] tracking-widest uppercase mt-0.5">Journey</p>
            </div>
          </div>
          <button 
