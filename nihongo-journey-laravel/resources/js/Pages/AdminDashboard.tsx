...
import { Users, CheckCircle, Book, Award, Eye, ShieldAlert, Search } from 'lucide-react';
...
export default function AdminDashboard({ usersData = [], stats }: AdminDashboardProps) {
    ...
    >
      <header className="border-b border-[#E5E5E5] pb-6 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <ShieldAlert className="text-[var(--color-japan-red)]" size={32} />
            <h1 className="font-serif text-4xl font-light">Dashboard Admin</h1>
          </div>
          <p className="text-[var(--color-ink-light)]">Pantau aktivitas, progres latihan, sertifikasi, dan catatan seluruh pengguna.</p>
        </div>
        <div className="bg-[var(--color-japan-red)]/10 text-[var(--color-japan-red)] px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider border border-[var(--color-japan-red)]/20">
          Mode Administrator
        </div>
      </header>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-[#E5E5E5] flex items-center gap-6">
          ...
          <div>
            <p className="text-sm text-[var(--color-ink-light)] mb-1">Total Pengguna</p>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-[#E5E5E5] flex items-center gap-6">
          ...
          <div>
            <p className="text-sm text-[var(--color-ink-light)] mb-1">Kuis Dikerjakan</p>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-[#E5E5E5] flex items-center gap-6">
          ...
          <div>
            <p className="text-sm text-[var(--color-ink-light)] mb-1">Sertifikasi Lulus</p>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-[#E5E5E5] flex items-center gap-6">
          ...
          <div>
            <p className="text-sm text-[var(--color-ink-light)] mb-1">Catatan Ditulis</p>
      ...
      {/* Users Table Section */}
      <section className="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-[#E5E5E5] overflow-hidden">
        <div className="p-6 border-b border-[#E5E5E5] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h2 className="font-serif text-2xl font-medium">Daftar Pengguna & Progres</h2>
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-2.5 bg-[var(--color-washi)] rounded-full text-sm border-none focus:ring-2 focus:ring-[var(--color-japan-red)] focus:outline-none"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[var(--color-washi)] text-[var(--color-ink-light)] text-xs font-bold uppercase tracking-wider border-b border-[#E5E5E5]">
                ...
                <th className="py-4 px-6 text-center">Kuis</th>
                <th className="py-4 px-6 text-center">Sertifikasi</th>
                <th className="py-4 px-6 text-center">Catatan</th>
            ...
            <tbody className="divide-y divide-[#E5E5E5] text-sm">
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-4 px-6 text-center font-serif font-bold text-lg">
                    {user.quizzes.length}
                  </td>
                  ...
                  <td className="py-4 px-6 text-center font-serif font-bold text-lg text-blue-600">
                    {user.notes.length}
                  </td>
              ...
              {filteredUsers.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-[var(--color-ink-light)] text-sm">
      ...
      {selectedUser && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <motion.div 
            ...
            className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[85vh] flex flex-col overflow-hidden border border-[#E5E5E5]"
          >
            <div className="p-6 border-b border-[#E5E5E5] flex justify-between items-center bg-[var(--color-washi)]">
              ...
              <button 
                ...
                className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-gray-500 hover:text-black shadow-sm"
              >
                ✕
              </button>
            </div>
              ...
              <button
                ...
                className={`py-3 px-6 text-sm font-bold border-b-2 transition-all ${
                  activeModalTab === 'quizzes' 
                    ? 'border-[var(--color-japan-red)] text-[var(--color-japan-red)] bg-white rounded-t-xl shadow-sm' 
                    : 'border-transparent text-[var(--color-ink-light)] hover:text-black'
                }`}
              >
                ...
                <div className="space-y-3">
                  {selectedUser.quizzes.map((quiz) => (
                    <div key={quiz.id} className="p-4 rounded-2xl border border-[#E5E5E5] flex justify-between items-center bg-gray-50/50">
                      <div>
                        <span className="text-xs font-bold uppercase tracking-wider text-[var(--color-japan-red)] block mb-1">
                          {quiz.category}
                        </span>
                  ...
                  {selectedUser.quizzes.length === 0 && (
                    <p className="text-center py-8 text-gray-400 text-sm">Belum ada riwayat kuis.</p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-3">
                  {selectedUser.certifications.map((cert) => (
                    <div key={cert.id} className="p-4 rounded-2xl border border-[#E5E5E5] flex justify-between items-center bg-gray-50/50">
                      <div>
                        <span className="text-sm font-bold uppercase tracking-wider text-[var(--color-ink)] block">
                        ...
                        <span className={`text-xs font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full inline-block mt-1 ${
                          cert.passed ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                        }`}>
                  ...
                  {selectedUser.certifications.length === 0 && (
                    <p className="text-center py-8 text-gray-400 text-sm">Belum ada riwayat sertifikasi.</p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-4">
                  {selectedUser.notes.map((note) => (
                    <div key={note.id} className="p-5 rounded-2xl border border-[#E5E5E5] bg-[var(--color-washi)]/30 space-y-2">
                      <span className="text-xs font-bold uppercase tracking-wider text-[var(--color-japan-red)] bg-[#fff9f9] px-3 py-1 rounded-full inline-block">
                        {note.date}
                      </span>
                  ...
                  {selectedUser.notes.length === 0 && (
                    <p className="text-center py-8 text-gray-400 text-sm">Belum ada catatan.</p>
                  )}
                </div>
              )}
            </div>
...
AdminDashboard.layout = (page: React.ReactNode) => <Layout>{page}</Layout>;
