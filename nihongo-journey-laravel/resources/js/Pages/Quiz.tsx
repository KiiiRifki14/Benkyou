...
export default function Quiz() {
  ...
  const generateQuestions = () => {
    ...
    shuffledGrammar.slice(0, 3).forEach(g => {
      ...
      if (others.length < 3) {
        others = ['私は学生です。', '犬じゃありません。', 'ペンですか。'].slice(0, 3 - others.length).concat(others);
      }

      mixedQuestions.push({
        text: `Bagaimana cara mengatakan "${example.en}" dalam bahasa Jepang?`,
        options: [...others, example.jp].sort(() => 0.5 - Math.random()),
        answer: example.jp,
        category: 'Grammar'
      });
    });

    setQuestions(mixedQuestions.sort(() => 0.5 - Math.random()));
    resetState();
  };
  ...
  if (showResult) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-md mx-auto text-center space-y-8 py-12 px-2 sm:px-4">
        <div className="bg-white p-6 sm:p-12 rounded-3xl shadow-xl border border-[#E5E5E5] space-y-6">
          <h2 className="text-2xl sm:text-3xl font-serif">Hasil Latihan</h2>
          <div className="text-5xl sm:text-6xl font-bold text-[var(--color-japan-red)]">
            {score} <span className="text-xl sm:text-2xl text-gray-400">/ {questions.length}</span>
          </div>
          <p className="text-[var(--color-ink-light)] text-sm sm:text-base">Kerja bagus, kamu telah berlatih kemampuan bahasa Jepangmu!</p>
          <button 
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-12 px-2 sm:px-4">
      <header className="text-center space-y-4 mb-8 sm:mb-10">
        <h1 className="font-serif text-3xl sm:text-4xl font-light">Latihan Tanpa Batas</h1>
        <p className="text-[var(--color-ink-light)] max-w-xl mx-auto text-sm sm:text-base">
          Kamu bisa mengulang latihan ini berkali-kali! Setiap sesi akan mengacak 10 pertanyaan dari Kosakata, Kanji, dan Tata Bahasa.
        </p>
      </header>
      ...
      <div className="flex justify-between items-center bg-white p-3 sm:p-4 rounded-2xl border border-[#E5E5E5]">
        <div className="flex items-center gap-2 max-w-[60%]">
          ...
          <span className="text-xs sm:text-sm font-bold uppercase tracking-wider truncate">{q.category}</span>
        </div>
      ...
      <div className="bg-white p-6 sm:p-8 md:p-12 rounded-3xl shadow-lg border border-[#E5E5E5] space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 gap-3 sm:gap-4">
          {q.options.map((opt, idx) => {
            ...
            let variant = "border-[#E5E5E5] hover:border-[var(--color-ink)]";
            if (answered) {
              if (isCorrect) variant = "border-green-500 bg-green-50 text-green-700 ring-1 ring-green-200";
              else if (isSelected) variant = "border-red-400 bg-red-50 text-red-600";
            ...
            return (
              ...
              >
                ...
                {answered && isCorrect && <CheckCircle2 size={18} className="text-green-500 flex-shrink-0" />}
        ...
        {answered && (
          ...
          >
            {currentQuestion === questions.length - 1 ? 'Selesaikan' : 'Soal Berikutnya'} <ChevronRight size={18} />
          </motion.button>
        )}
      </div>
    </div>
