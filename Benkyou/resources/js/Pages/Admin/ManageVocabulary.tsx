...
import { Plus, Edit2, Trash2, Search, List, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export default function ManageVocabulary({ vocabulariesData = [] }: ManageVocabularyProps) {
  ...
  const [activeType, setActiveType] = useState<'semua' | 'noun' | 'verb' | 'adjective' | 'greeting' | 'adverb'>('semua');
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<Vocabulary | null>(null);
  ...
  const handleTypeChange = (type: 'semua' | 'noun' | 'verb' | 'adjective' | 'greeting' | 'adverb') => {
    setActiveType(type);
    setCurrentPage(1);
  };
  ...
  const filteredVocabs = vocabulariesData.filter(v => {
    ...
    const matchesType = activeType === 'semua' || v.type.toLowerCase() === activeType.toLowerCase();
    
    return matchesSearch && matchesType;
  });

  const totalPages = Math.max(1, Math.ceil(filteredVocabs.length / ROWS_PER_PAGE));
  const paginatedVocabs = filteredVocabs.slice((currentPage - 1) * ROWS_PER_PAGE, currentPage * ROWS_PER_PAGE);
  ...
  const executeDelete = () => {
    ...
    router.delete(`/admin/vocabulary/${deleteConfirm.id}`, {
      onSuccess: () => {
  ...
  const getTypeBadgeStyles = (type: string) => {
    ...
    if (t === 'noun') return 'bg-blue-50 text-blue-700 border border-blue-100';
    if (t === 'verb') return 'bg-emerald-50 text-emerald-700 border border-emerald-100';
    if (t === 'adjective') return 'bg-amber-50 text-amber-700 border border-amber-100';
    if (t === 'greeting') return 'bg-purple-50 text-purple-700 border border-purple-100';
    return 'bg-indigo-50 text-indigo-700 border border-indigo-100'; // adverb / default
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-2xl font-bold text-slate-800 flex items-center gap-2">
            <List className="text-[#bc002d]" size={28} />
            Kelola Kosakata
          </h1>
          <p className="text-slate-500 text-sm mt-0.5">Tambah, ubah, dan hapus data Kosakata Benkyou.</p>
        </div>
        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border text-[#bc002d] border-red-100 bg-red-50 shrink-0">
          {vocabulariesData.length} Kosakata
        </span>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-400 text-[11px] font-bold uppercase tracking-wider border-b border-slate-100">
                <th className="py-3.5 px-6">Kata Jepang</th>
                <th className="py-3.5 px-6">Cara Baca (Romaji)</th>
                <th className="py-3.5 px-6">Arti / Makna</th>
                <th className="py-3.5 px-6 text-center">Golongan Kata</th>
              ...
              {paginatedVocabs.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-16 text-center">
                    ...
                    <p className="text-slate-400 text-sm font-medium">Tidak ada data Kosakata yang ditemukan.</p>
                    <button
                      onClick={() => openCrud('add')}
                ...
                paginatedVocabs.map((v) => (
                  <tr key={v.id} className="hover:bg-slate-50/70 transition-colors">
                    <td className="py-3.5 px-6 font-jp font-bold text-2xl text-slate-800">{v.word}</td>
                    <td className="py-3.5 px-6 font-mono font-bold text-base text-[#bc002d]">{v.romaji}</td>
                    <td className="py-3.5 px-6 font-medium text-slate-600">{v.meaning}</td>
                    <td className="py-3.5 px-6 text-center">
                      <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wider uppercase ${getTypeBadgeStyles(v.type)}`}>
                        {v.type}
                      </span>
                    </td>
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between text-sm bg-white">
            <span className="text-slate-400 text-xs">
              {((currentPage - 1) * ROWS_PER_PAGE) + 1}–{Math.min(currentPage * ROWS_PER_PAGE, filteredVocabs.length)} dari {filteredVocabs.length}
            </span>
            ...
            >
              <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 className="font-bold text-slate-800 text-lg">
                  {crudModal.mode === 'add' ? 'Tambah' : 'Ubah'} Kosakata
                </h3>
                <button
              ...
              <form onSubmit={handleCrudSubmit} className="p-6 space-y-4 bg-white">
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1">Kata Jepang (Karakter / Kanji)</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1">Arti / Terjemahan</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-1">Golongan Kata</label>
                  <select 
          ...
          >
            ...
            >
              <div className="p-6 text-center space-y-4">
                ...
                <div>
                  <h3 className="font-bold text-slate-800 text-lg">Hapus Kosakata?</h3>
                  <p className="text-sm text-slate-500 mt-1">
                    Apakah Anda yakin ingin menghapus <strong>{deleteConfirm.word} ({deleteConfirm.romaji})</strong>? Tindakan ini tidak dapat dibatalkan.
                  </p>
                </div>
