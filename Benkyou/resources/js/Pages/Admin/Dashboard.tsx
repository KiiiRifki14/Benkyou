...
import { 
  Users, CheckCircle, Book, Award, Eye, Search, 
  TrendingUp, Plus, X, ChevronLeft, ChevronRight,
  PenTool, Languages, List, BookOpen, FileQuestion, GraduationCap
} from 'lucide-react';
...
export default function Dashboard({ usersData = [], stats }: DashboardProps) {
  ...
  const quickLinks = [
    { label: 'Tambah Kana', icon: PenTool, href: '/admin/kana', color: '#bc002d' },
    { label: 'Tambah Kanji', icon: Languages, href: '/admin/kanji', color: '#7c3aed' },
    { label: 'Tambah Kosakata', icon: List, href: '/admin/vocabulary', color: '#0891b2' },
    { label: 'Tambah Grammar', icon: BookOpen, href: '/admin/grammar', color: '#059669' },
    { label: 'Tambah Soal', icon: FileQuestion, href: '/admin/question', color: '#d97706' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-2xl font-bold text-slate-800">Pusat Administrator</h1>
          <p className="text-slate-500 text-sm mt-0.5">Kelola data pembelajaran & pantau progres siswa Benkyou.</p>
        </div>
          ...
          return (
            ...
            >
              <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 text-white"
                style={{ background: card.bg, boxShadow: `0 4px 14px ${card.color}40` }}>
                <Icon size={22} />
              </div>
              <div className="min-w-0">
                <p className="text-xs text-slate-500 mb-0.5 truncate">{card.label}</p>
                <p className="text-3xl font-serif font-bold text-slate-800 leading-none">{card.value}</p>
              </div>
              <TrendingUp size={60} className="absolute -right-3 -bottom-3 opacity-[0.04] text-slate-900" />
            </motion.div>
          );
        })}
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
        <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3">Aksi Cepat</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            ...
            <p className="text-xs text-slate-400">{filteredUsers.length} pengguna ditemukan</p>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-400 text-[11px] font-bold uppercase tracking-wider border-b border-slate-100">
                ...
                <th className="py-3 px-6 text-center">Lulus Ujian</th>
              ...
              {paginatedUsers.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-16 text-center">
                    <p className="font-jp text-4xl text-slate-200 mb-2">探す</p>
                ...
                paginatedUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3.5 px-6 text-center">
                      <span className="font-bold text-slate-700">{user.quizzes.length}</span>
                    </td>
                    <td className="py-3.5 px-6 text-center">
                      <span className="font-bold text-emerald-600">{user.certifications.filter(c => c.passed).length}</span>
                    </td>
                    <td className="py-3.5 px-6 text-center">
                      <span className="font-bold text-blue-600">{user.notes.length}</span>
                    </td>
                    <td className="py-3.5 px-6 text-center">
                      <button
                        onClick={() => { setSelectedUser(user); setActiveModalTab('quizzes'); }}
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between text-sm">
            <span className="text-slate-400 text-xs">
              {((currentPage - 1) * ROWS_PER_PAGE) + 1}–{Math.min(currentPage * ROWS_PER_PAGE, filteredUsers.length)} dari {filteredUsers.length}
            </span>
            ...
            >
              ...
              <div className="p-5 border-b border-slate-100 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-slate-800 text-lg leading-tight">{selectedUser.name}</h3>
              ...
              <div className="flex border-b border-slate-100 px-5 gap-1 bg-slate-50">
                {(['quizzes', 'certifications', 'notes'] as const).map(tab => (
                  <button
                    key={tab}
                  ...
                  >
                    {tab === 'quizzes' && `Kuis (${selectedUser.quizzes.length})`}
                    {tab === 'certifications' && `Sertifikasi (${selectedUser.certifications.length})`}
                    {tab === 'notes' && `Catatan (${selectedUser.notes.length})`}
                  </button>
                ))}
              </div>
              ...
              <div className="p-5 overflow-y-auto flex-1 space-y-3">
                {activeModalTab === 'quizzes' && (
                  selectedUser.quizzes.length === 0
                    ? <p className="text-center text-slate-400 py-8 text-sm">Belum ada riwayat kuis.</p>
                    : selectedUser.quizzes.map((quiz) => (
                      <div key={quiz.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50 flex justify-between items-center">
                        <div>
                          <span className="text-xs font-bold uppercase tracking-wider text-[#bc002d] block mb-1">{quiz.category}</span>
                          <span className="text-xs text-slate-400">{new Date(quiz.created_at).toLocaleDateString('id-ID', { dateStyle: 'medium' })}</span>
                        </div>
                        <div className="text-right">
                          <span className="font-bold text-xl text-slate-800">{quiz.score}</span>
                          <span className="text-slate-400 text-xs"> / {quiz.total}</span>
                          <div className="w-20 h-1.5 rounded-full bg-slate-200 mt-1 overflow-hidden">
                            <div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.round((quiz.score / quiz.total) * 100)}%` }} />
                          </div>
                        </div>
                      </div>
                    ))
                ...
                {activeModalTab === 'certifications' && (
                  selectedUser.certifications.length === 0
                    ? <p className="text-center text-slate-400 py-8 text-sm">Belum ada data sertifikasi.</p>
                    : selectedUser.certifications.map((cert) => (
                      <div key={cert.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50 flex justify-between items-center">
                        <div>
                          <span className="font-bold text-slate-800 block">Level {cert.category.toUpperCase()} — Tahap {cert.level}</span>
                          <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full inline-block mt-1 ${cert.passed ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'}`}>
                            {cert.passed ? '✓ Lulus' : '✗ Gagal'}
                          </span>
                        </div>
                        <span className="font-bold text-2xl text-slate-800">{cert.score}<span className="text-sm text-slate-400"> pts</span></span>
                      </div>
                    ))
                )}
                {activeModalTab === 'notes' && (
                  selectedUser.notes.length === 0
                    ? <p className="text-center text-slate-400 py-8 text-sm">Belum ada catatan pribadi.</p>
                    : selectedUser.notes.map((note) => (
                      <div key={note.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50 space-y-2">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#bc002d] bg-red-50 px-2.5 py-1 rounded-full inline-block">{note.date}</span>
