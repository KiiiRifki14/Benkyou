...
export default function MissionLevel({ meta, subLevels, userCerts }: MissionLevelProps) {
    return (
        <div className="max-w-4xl mx-auto space-y-8 pt-8">
            <Head title={`${meta.title} - Ujian Sertifikasi`} />
            ...
            <div className="bg-white rounded-[2rem] border border-[#E5E5E5] p-8 md:p-12 text-center">
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-[var(--color-ink)] mb-2">{meta.title}</h1>
                <p className="text-[var(--color-ink-light)] text-lg mb-6">{meta.subtitle}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {subLevels.length === 0 && (
                    <div className="col-span-1 md:col-span-2 text-center py-16 bg-white rounded-3xl border border-dashed border-[#E5E5E5]">
                        <p className="text-[var(--color-ink-light)] font-medium">Belum ada soal untuk level ini.</p>
                    </div>
                )}
            </div>
        </div>
