...
export default function Certification({
    ...
    const [selectedCategory, setSelectedCategory] =
        useState<CertificationCategory | null>(null);
    const [selectedLevel, setSelectedLevel] =
        useState<CertificationLevel | null>(null);

    useEffect(() => {
        if (questionsData && questionsData.length > 0) {
            const mappedCategories = certificationCategories.map((cat) => {
                const catQuestions = questionsData.filter(
                    (q) => q.type === cat.id,
                );
                ...
                const newLevels = Object.keys(levelsMap)
                    .map((lvlIdStr) => {
    ...
    useEffect(() => {
        if (selectedCategory) {
            ...
            if (updatedCat) {
                ...
                if (selectedLevel) {
                    const updatedLvl = updatedCat.levels.find(
    ...
    const [isTypingCorrect, setIsTypingCorrect] = useState<boolean | null>(
        null,
    );
    ...
    const selectLevelAction = (lvl: CertificationLevel) => {
        if (
            selectedCategory &&
            unlockedLevels[selectedCategory.id].includes(lvl.id)
        ) {
            setSelectedLevel(lvl);
            setStarted(false);
            setPreparedQuestions(null); // reset prepared questions when changing level
            setShowResult(false);
        }
    };
    ...
    const startQuiz = () => {
        ...
        setShowResult(false);
        resetQuestionState();
        // prepare questions for this run: shuffle questions order (within level) and shuffle options per question
        if (selectedLevel) {
            const clone = selectedLevel.questions.map((q) => ({ ...q }));
            ...
            // shuffle options for each question (leave typing answers as-is)
            const prepared = clone.map((q) => {
                if (
                    q.options &&
            ...
            setPreparedQuestions(shuffleArray(prepared));
        }
    };
    ...
    const handleMultipleChoiceAnswer = (option: string) => {
        ...
        const isCorrect = Array.isArray(q.answer)
            ...
            : normalize((q.answer as string) || "") === normalize(option);
    ...
    const nextQuestion = () => {
        ...
        if (next < total) {
            ...
            if (
                isPassed &&
                !unlockedLevels[selectedCategory.id].includes(
                    selectedLevel.id + 1,
                )
            ) {
    ...
    const playAudio = (text: string, lang: string = "ja-JP") => {
        try {
            if ("speechSynthesis" in window) {
                ...
                alert(
                    "Mohon maaf, fitur audio (Text-to-Speech) tidak didukung di browser ini.",
                );
            }
        } catch (err) {
            console.error(err);
    ...
    const passed = selectedLevel
        ? percentage >= selectedLevel.passingScore
        : false;

    const handleResetProgress = () => {
        if (
            window.confirm(
                "Apakah kamu yakin ingin mereset progres untuk kategori ini?",
            )
        ) {
            if (selectedCategory) {
                setUnlockedLevels((prev) => ({
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
                    {categories.map((cat) => {
                        ...
                        const isCompleted = unlocked.includes(
                            cat.levels.length + 1,
                        );
        ...
        return (
            ...
            >
                <button
                    ...
                    className="flex items-center gap-2 text-[var(--color-ink-light)] hover:text-black transition-colors mb-4 -mt-4"
                >
                ...
                <header className="text-center space-y-4 mb-10">
                    ...
                    <div className="flex items-center justify-center gap-4 mt-6">
                        <button
                            ...
                            className="text-sm font-medium text-red-500 hover:text-red-700 underline underline-offset-4"
                        >
                ...
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                    {selectedCategory.levels.map((lvl) => {
                        const isUnlocked = unlockedLevels[
                            selectedCategory.id
                        ].includes(lvl.id);
                        return (
                            <button
                            ...
                            >
                                <div
                                    className={`w-12 h-12 shrink-0 rounded-full flex items-center justify-center text-white ${isUnlocked ? "shadow-sm" : "bg-gray-300"}`}
                                    style={{
        ...
        return (
            ...
            >
                <button
                    ...
                    className="flex items-center gap-2 text-[var(--color-ink-light)] hover:text-black transition-colors mb-4"
                >
    ...
    if (showResult && selectedLevel && selectedCategory) {
        const isFullCompletion =
            passed && selectedLevel.id === selectedCategory.levels.length;
            ...
            >
                {passed ? (
                    <div className="space-y-6">
                        ...
                        <div
                            className={`py-8 border-y-2 border-dashed relative overflow-hidden rounded-xl mt-8 shadow-inner px-4 md:px-0`}
                            style={{
                                borderColor: selectedCategory.themeColor,
                        ...
                        >
                            ...
                            {isFullCompletion ? (
                                <>
                                    ...
                                    <div
                                        className="font-serif text-2xl sm:text-3xl md:text-4xl font-bold mb-2 tracking-wide text-balance"
                                        style={{
                                    ...
                                    <div className="my-6 p-4 bg-white/60 mx-4 rounded-xl border border-white/40 shadow-sm backdrop-blur-sm">
                                        <div className="text-xs md:text-sm font-bold text-gray-800 mb-2 uppercase tracking-wide">
                                            🏆 Tema Hadiah Terbuka!
                                        </div>
                                        <div
                    ...
                    <div className="space-y-6">
                        ...
                        <p className="text-[var(--color-ink-light)]">
                            Skor kamu {score}/{totalQuestions} ({percentage}%).
                            Kamu butuh {selectedLevel.passingScore}% untuk
                            lulus.
                        </p>
                        <p className="text-sm italic">
                            "Nanakorobi yaoki" — Jatuh tujuh kali, bangkit
                            delapan kali.
                        </p>
                        <button
                            onClick={startQuiz}
            ...
            <div className="bg-white p-6 md:p-10 rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-[#E5E5E5] space-y-8">
                ...
                {q.type === "listening" && q.spokenText && (
                    <div className="flex flex-col items-center justify-center space-y-4 py-10 bg-[#f8f9fa] rounded-2xl border border-dashed border-[#d1d5db]">
                        <button
                            onClick={() =>
                                playAudio(q.spokenText!, q.speechLang)
                            }
                        ...
                        <p className="text-sm font-bold text-[var(--color-ink-light)] tracking-widest uppercase animate-pulse">
                            Klik untuk Mendengar Audio
                        </p>
                    </div>
                )}
                    ...
                    q.options && (
                        <div className="grid grid-cols-1 gap-4">
                            {q.options.map((opt, idx) => {
                                let colorClass =
                                    "border-[#E5E5E5] hover:border-[var(--color-japan-red)] hover:bg-[#fff9f9]";
                                if (answered) {
                                    ...
                                    if (isCorrectOpt) {
                                        colorClass =
                                            "border-green-500 bg-green-50 text-green-800 ring-2 ring-green-200 font-bold";
                                    } else if (isSelected) {
                                        colorClass =
                                ...
                                return (
                                    <button
                                        key={idx}
                                        onClick={() =>
                                            handleMultipleChoiceAnswer(opt)
                                        }
                                        disabled={answered}
                                    ...
                                    >
                                        <span className="text-base sm:text-lg pr-8 block">
                                            {opt}
                                        </span>
                                        {answered &&
                                            (Array.isArray(q.answer)
                                                ...
                                                : opt.trim().toLowerCase() ===
                                                  (q.answer as string)
                                                      .trim()
                                                      .toLowerCase()) && (
                                                <span className="absolute right-3 sm:right-5 top-1/2 -translate-y-1/2 text-green-500">
                                            ...
                                            !(Array.isArray(q.answer)
                                                ...
                                                : opt.trim().toLowerCase() ===
                                                  (q.answer as string)
                                                      .trim()
                                                      .toLowerCase()) && (
                                                <span className="absolute right-3 sm:right-5 top-1/2 -translate-y-1/2 text-red-500">
                                                    <XCircle size={24} />
                                                </span>
                                            )}
                                    </button>
                ...
                {q.type === "typing" && (
                    <div className="flex flex-col space-y-4 max-w-md mx-auto w-full">
                        <div
                            className={`flex bg-white border-2 rounded-2xl overflow-hidden transition-colors ${answered ? "border-[#E5E5E5]" : "border-[var(--color-ink-light)] focus-within:border-[var(--color-japan-red)]"}`}
                        >
                            <input
                                type="text"
                                value={typingInput}
                        ...
                        {answered && (
                            <motion.div
                                ...
                                className={`p-4 rounded-xl flex items-center gap-3 ${isTypingCorrect ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                            >
                                {isTypingCorrect ? (
                                    <>
                                        <CheckCircle2 size={24} />{" "}
                        ...
                        {answered && !isTypingCorrect && (
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="p-4 bg-gray-50 rounded-xl border border-dashed border-[#ccc] text-center"
                            >
